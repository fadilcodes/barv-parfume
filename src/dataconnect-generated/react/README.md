...
## AllPerfumes
You can execute the `AllPerfumes` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useAllPerfumes(dc: DataConnect, options?: useDataConnectQueryOptions<AllPerfumesData>): UseDataConnectQueryResult<AllPerfumesData, undefined>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useAllPerfumes(options?: useDataConnectQueryOptions<AllPerfumesData>): UseDataConnectQueryResult<AllPerfumesData, undefined>;
```

### Variables
The `AllPerfumes` Query has no variables.
### Return Type
Recall that calling the `AllPerfumes` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `AllPerfumes` Query is of type `AllPerfumesData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `AllPerfumes`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig } from '@dataconnect/generated';
import { useAllPerfumes } from '@dataconnect/generated/react'

export default function AllPerfumesComponent() {
  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useAllPerfumes();

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useAllPerfumes(dataConnect);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useAllPerfumes(options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useAllPerfumes(dataConnect, options);
  ...
  if (query.isSuccess) {
    console.log(query.data.perfumes);
  }
...
## UserLists
You can execute the `UserLists` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useUserLists(dc: DataConnect, options?: useDataConnectQueryOptions<UserListsData>): UseDataConnectQueryResult<UserListsData, undefined>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useUserLists(options?: useDataConnectQueryOptions<UserListsData>): UseDataConnectQueryResult<UserListsData, undefined>;
```

### Variables
The `UserLists` Query has no variables.
### Return Type
Recall that calling the `UserLists` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `UserLists` Query is of type `UserListsData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `UserLists`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig } from '@dataconnect/generated';
import { useUserLists } from '@dataconnect/generated/react'

export default function UserListsComponent() {
  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useUserLists();

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useUserLists(dataConnect);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useUserLists(options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useUserLists(dataConnect, options);
...
export default function CreateReviewComponent() {
  ...
  mutation.mutate({ perfumeId: ..., rating: ..., reviewText: ..., isPublic: ..., });
...
export default function UpdateUserProfileComponent() {
  ...
  // Variables can be defined inline as well.
  mutation.mutate({ displayName: ..., bio: ..., photoUrl: ..., });
